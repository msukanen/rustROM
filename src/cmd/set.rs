...
use crate::{cmd::{Command, CommandCtx}, resume_game, tell_unknown_command, tell_user, ClientState};
...
\n"));
            ...
            tell_user!(ctx.writer, format!("Usage: set <field> <value>\n{}", ctx.prompt));
            resume_game!(ctx);
        }
        ...
        if field.eq_ignore_ascii_case("ro") {
            if value.eq_ignore_ascii_case("greeting") {
                ...
                if let Some(g) = g {
                    tell_user!(ctx.writer, format!("---greeting, current value:---\n{}\n", g));
                } else {
